...
export default class SteamGameRunner_Windows extends GameRunnerProvider {
    ...
    async start(game: Game, args: string[]): Promise<void | R2Error> {
        return new Promise(async (resolve, reject) => {
            ...
            LoggerProvider.instance.Log(LogSeverity.INFO, `Running command: ${gameDir}.exe -applaunch ${game.activePlatform.storeIdentifier} ${mappedArgs} ${settings.getContext().gameSpecific.launchParameters}`);
            
            ChildProcess.exec(`"${gameDir}/${game.exeName}" -applaunch ${game.activePlatform.storeIdentifier} ${mappedArgs} ${settings.getContext().gameSpecific.launchParameters}`, undefined, (err => {
                if (err !== null) {
